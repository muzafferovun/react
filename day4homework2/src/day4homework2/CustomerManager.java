...
public class CustomerManager {
	...
	public void listCustomers(Customer[] customers) {
		for(Customer customer:customers) {
			if(customer.getId()>-1) {
				System.out.println("Müşteri :"+customer.getId());
				System.out.println("      İsim :"+customer.getFirstName()+" "+customer.getLastName());
	...
	public void listStars(Customer[] customers) {
		System.out.println("Yıldızlı Müşteriler :");
		String star="";
		int i;
		for(Customer customer:customers) {
			if(customer.getId()>-1) {
				...
				System.out.println("    İsim :"+customer.getFirstName()+" "+customer.getLastName()+" ( "+star+" ) ");
